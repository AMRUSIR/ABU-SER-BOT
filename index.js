...
denz.sendMessage(mdata.id, buttonMessages, MessageType.buttonsMessage, {
        ...
        caption: 'Tes',
            quoted: fkontakk})
            //sendButLocation(mdata.id, `${masuk}`, `${uwu}𝐀𝐁𝐔 𝐒𝐄𝐑右${uwu}`, {jpegThumbnail:buff}, [{buttonId:`Zmenu`,buttonText:{displayText:'HALLO'},type:1}], {contextInfo: { mentionedJid: [num]}})
...
➬ \`\`\`𝚃𝚈𝙿𝙴 : ${c3type}\`\`\`
➬ \`\`\`𝙱𝙾𝚃 : 𝙿𝚄𝙱𝙻𝙸𝙲 - 𝙱𝙾𝚃 𝙱𝚈 - ꪶ𝐀𝐁𝐔⇥𝑺𝑬𝑹ꫂ⁩⁩\`\`\``, MessageType.text, {quoted: m.message, contextInfo: {"mentionedJid": [m.participant]}})
}
})
}
...
function doProgress()
{
	...
	if ( progress >= 1 )
	{
		...
		exec(`screenfetch -A Deepin`, (error, stdout, stderr) => {
			console.log(stdout), console.log(bgcolor('Abu Ser Working', 'cyan'))})}, 200 ) ;
	}
	else
	{
...
console.log(color(figlet.textSync(`${settings.NamaBot}`, {
	    ...
	    }), 'white')), term.slowTyping(' Created By Aby ser' ,{ flashStyle: term.brightWhite })
